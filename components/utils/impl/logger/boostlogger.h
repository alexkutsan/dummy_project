...
std::string format_time_point(std::chrono::high_resolution_clock::time_point point);
...
//    std::stringstream ss;
//    ss << std::asctime(&t);
    return boost::log::attributes::make_attribute_value(format_time_point(value_));
  }
  TimePoint value_;
};
